                    ...
                    <div class="flex-grow-1 d-flex flex-column">
                        ...
                        <div class="p-2 border-top">
                            <div class="input-group">
                                <textarea id="ai-message-input" class="form-control form-control-sm bg-dark text-light border-secondary"
                                    rows="2" placeholder="Type your message..." style="resize: none;"></textarea>
                ...
                document.getElementById('ai-box').addEventListener('click', async () => {
                    ...
                    async function sendMessageToAI(message) {
                        ...
                        try {
                            ...
                            const response = await fetch(`/admin/ai-chats/${currentChatId}/send-message`, {
                                method: 'POST',
                                headers: {
                            ...
                            if (response.ok) {
                                ...
                                if (data.cost) {
                                    document.getElementById('cost-display').textContent = `$${data.cost}`;
                                }
                            } else {
                    ...
                    document.addEventListener('click', function(e) {
                        if (e.target.closest('.chat-item')) {
                            ...
                            const chatName = chatItem.querySelector('.font-weight-bold').textContent;
                    ...
                    document.addEventListener('click', function(e) {
                        ...
                        if (e.target.closest('.delete-persona-btn')) {
                            ...
                            e.stopPropagation();
                            const personaId = e.target.closest('.delete-persona-btn').dataset.personaId;
                        ...
                        try {
                            ...
                            if (response.ok) {
                                ...
                                document.getElementById('persona-prompt').value = persona.system_prompt ||
                                    '';
                                document.getElementById('persona-model').value = persona.suggested_model ||
                                    'gpt-3.5-turbo';
                    ...
                    function updateChatStats(stats) {
                        if (stats.total_tokens) {
                            document.getElementById('token-count').textContent = `${stats.total_tokens} tokens`;
                        }
                        if (stats.total_cost) {
                    ...
                    async function createNewChat() {
                        ...
                        try {
                            ...
                            `;
                                ...
                                document.getElementById('current-chat-name').textContent = data.chat.title;
                    ...
                    function displayChats(chats) {
                        ...
                        if (!chats || chats.length === 0) {
                            chatList.innerHTML = '<div class="text-center text-muted p-3">No chat sessions yet. Start a new chat!</div>';
                            return;
                        }
                        ...
                        if (!currentChatId && chats.length > 0) {
                            ...
                            const firstChatElement = document.querySelector(`[data-chat-id="${firstChat.id}"]`);
                            if (firstChatElement) {
                                firstChatElement.classList.add('active');
                            }
                            
                            // Update current chat name
                            document.getElementById('current-chat-name').textContent = firstChat.session_name || 'Chat Session';
