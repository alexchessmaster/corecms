<div class="form-group">
    ...
    <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name"
        value="{{ old('name', isset($bookAuthor) ? $bookAuthor->getTranslation('name', app()->getLocale(), false) : '') }}"
...
<div class="form-group">
    ...
    <input type="date" class="form-control @error('date_of_birth') is-invalid @enderror" id="date_of_birth"
        name="date_of_birth"
        value="{{ old('date_of_birth', isset($bookAuthor) && $bookAuthor->date_of_birth ? $bookAuthor->date_of_birth->format('Y-m-d') : '') }}">
    @error('date_of_birth')
        <div class="invalid-feedback">{{ $message }}</div>
    @enderror
...
<div class="form-group">
    ...
    <input type="date" class="form-control @error('date_of_death') is-invalid @enderror" id="date_of_death"
        name="date_of_death"
        value="{{ old('date_of_death', isset($bookAuthor) && $bookAuthor->date_of_death ? $bookAuthor->date_of_death->format('Y-m-d') : '') }}">
    @error('date_of_death')
        <div class="invalid-feedback">{{ $message }}</div>
    @enderror
...
<div class="form-group">
    ...
    <input type="text" class="form-control @error('nationality') is-invalid @enderror" id="nationality"
        name="nationality"
        value="{{ old('nationality', isset($bookAuthor) ? $bookAuthor->getTranslation('nationality', app()->getLocale(), false) : '') }}"
...
<div class="form-group">
    ...
    <textarea class="form-control @error('biography') is-invalid @enderror" id="biography" name="biography" rows="5"
        placeholder="Enter author biography">{{ old('biography', isset($bookAuthor) ? $bookAuthor->getTranslation('biography', app()->getLocale(), false) : '') }}</textarea>
    @error('biography')
        <div class="invalid-feedback">{{ $message }}</div>
    @enderror
...
<div class="form-group">
    ...
    @if (isset($bookAuthor) && $bookAuthor->image)
        <div class="mb-2">
            <img src="{{ $bookAuthor->image }}" alt="Current Author Image"
